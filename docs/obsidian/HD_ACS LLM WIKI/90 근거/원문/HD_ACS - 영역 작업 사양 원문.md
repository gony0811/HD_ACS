...
기준일: 2026-09-07. 저장소 원문: `docs/SPEC_AREA_TASK_MANUAL.md`. SHA256: `bb25c1bd861b615abdbc51e1f101d74dfa77009e397cbe776d24db2dd6bf0b2b`.
...
층 소속은 영역이 가진다). wall_code는 `TANK_WALL_LAYOUT.md` §2 의 naming rule을 따른다
(대외 정본은 `surface_id_enum.docx`). 아래 표가 현행 구현(`TankGeometry.GenerateWalls()`)과 일치한다.
초안의 잠정 코드(FL/BC-S/BC-P/SW-S/SW-P/TC-S/TC-P/CL/FW/AW)는 폐기되었으며 참고용으로만 병기한다.

| wall_code | Surface ID | 면 | P0 | U | V | n(내부향) | (구 잠정 코드) |
|---|---|---|---|---|---|---|---|
| `B` | 1 | 바닥 | (−L/2, −W_f/2, 0) | +x | +y | (0,0,+1) | FL |
| `SL` | 8 | 하부 챔퍼 우현(−y) | (−L/2, −W_f/2, 0) | +x | (0,−cosθ_l,+sinθ_l) | (0,+sinθ_l,+cosθ_l) | BC-S |
| `PL` | 7 | 하부 챔퍼 좌현(+y) | (−L/2, +W_f/2, 0) | +x | (0,+cosθ_l,+sinθ_l) | (0,−sinθ_l,+cosθ_l) | BC-P |
| `SM` | 4 | 수직벽 우현 | (−L/2, −B/2, h_low) | +x | +z | (0,+1,0) | SW-S |
| `PM` | 3 | 수직벽 좌현 | (−L/2, +B/2, h_low) | +x | +z | (0,−1,0) | SW-P |
| `SU` | 10 | 상부 챔퍼 우현 | (−L/2, −B/2, h_low+h_wall) | +x | (0,+cosθ_u,+sinθ_u) | (0,+sinθ_u,−cosθ_u) | TC-S |
| `PU` | 9 | 상부 챔퍼 좌현 | (−L/2, +B/2, h_low+h_wall) | +x | (0,−cosθ_u,+sinθ_u) | (0,−sinθ_u,−cosθ_u) | TC-P |
| `T` | 2 | 천장 | (−L/2, −W_ceil/2, H) | +x | +y | (0,0,−1) | CL |
| `F` | 5 | 선수 격벽 (x=+L/2) | (+L/2, +B/2, 0) | −y | +z | (−1,0,0) | FW |
| `A` | 6 | 선미 격벽 (x=−L/2) | (−L/2, −B/2, 0) | +y | +z | (+1,0,0) | AW |

- 격벽 2면은 평면·수직으로 가정(팔각 윤곽 전체가 한 평면)하며, `u_len × v_len = B × H` 직사각형으로
  저장한다. 실제 팔각 윤곽은 렌더링 단계에서 선창 파라미터로 재구성한다 — `TANK_RENDERING.md` §4.
  이 가정이 실선과 다르면 후속 개정.
- ⚠ **`A`(선미 격벽) U축 방향 미확정.** 위 표(=현행 구현)의 `A`는 U = +y 로 **우현 → 좌현**이지만,
  대외 정본 `surface_id_enum.docx` 와 비전 인터페이스 v3 §5 는 `F`·`A` 둘 다 **좌현 → 우현**으로
  적고 있다. 표의 값은 §1의 "마구리 면은 선창 내부에서 봤을 때 왼→오른쪽" 규칙을 따른 결과이며
  내부적으로는 일관하나, 정본과는 어긋난다. 어느 쪽으로 맞출지 결정 후 3자(ACS·비전·세이지) 동시
  반영이 필요하다. 미해결 상태에서는 후벽 촬영 좌표의 u가 좌우 반전되어 해석될 수 있다.
