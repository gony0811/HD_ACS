# SPEC — 선창 파라메트릭 벽면 정의 · 영역/검사 작업 수동 등록 (PHASE 2 개정 v3)

작성일: 2026-08-04 (v3 — 파라메트릭 선창 정의 + 벽면 로컬 좌표로 개정, v1/v2 대체) · 상태: 확정, 구현 대상
...
| GET | `/api/tanks/{tankId}/walls` | 자동 생성된 면 목록 (프레임·크기·facing_yaw 포함) |
| POST | `/api/areas` | 영역 등록 (u,v — 면 범위·경계 검증) |
...
- **영역/작업 섹션**: 면(wall_code) 선택 → 해당 면의 로컬 (u,v) 평면 캔버스(면 크기 u_len×v_len
  auto-fit) 위에 영역 사각형·정차점 마커(투영 위치)·작업 선분(방향 마커+seq 배지) 렌더.
  등록 폼: 영역(이름/level/u·v 경계/정차 수동 지정 체크), 작업(시작·끝 u,v/DXF/프로파일).
  FL·CL 면 선택 시 정차 수동 입력을 필수로 활성화.
