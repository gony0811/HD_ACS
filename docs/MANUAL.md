...
- 3D 뷰의 로봇 마커(빨간 원)에는 바닥면 **방향 화살표**가 붙는다 — AMR이 보고한 heading(VDA `agvPosition.theta`)을 층 캘리브레이션(T_W_D) yaw로 보정한 도면 방향. theta를 보고하지 않는 동안(부팅 직후 등)은 원만 보인다. 같은 값은 로봇 상태 카드 "방향(도면 x축 기준)"에 도 단위로 표시된다.
