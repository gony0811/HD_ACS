...
- 2026-08-19: Q2(검사 S/W 위치/시각 키 규약) 해소 — **ADR-013** 신설로 검사 S/W 결과 대조 규약 확정. 대조 모델=오프라인 사후 대조(ACS↔검사S/W 직접 API 없음, 경계 유지). **1차 키=`jobRef`**(상관 ID `JOB-{tank}-{level}-{wall}-{seam}-{seq}`, startWeldInspection payload로 HD_AMR 전달→검사 트리거 시 검사 S/W에 부기). **2차 키=위치+시각**: 위치=**도면 좌표계(Drawing frame, m)** `(tank,level,wall_code)`+seam 시작점 `x,y,z`(월드/맵 좌표는 맵버전 의존이라 대조 키에서 배제), 시각=종료 state timestamp(로봇 클록, UTC, ±2s 허용, 단일 NTP 소스). 폴백 대조=동일 벽면 AND 도면좌표 ε≤0.05m AND |Δt|≤2s. 스키마: `hist.inspection_result.job_ref` 컬럼+`ix_inspresult_job` 인덱스 추가(비파괴, `db/migrations/2026-08-19_q2_inspection_reconciliation.sql`), `InspectionResultEntity.JobRef`+DbContext 인덱스. 문서: ADR-004 Q2 해소 참조·ADR-008 좌표계 bullet 부분해소·Q 테이블 Q2 ✅·DB_SCHEMA·INSPECTION_SCENARIO·TANK_WALL_LAYOUT 참조 갱신. 협의 확인 필요: ① HD_AMR→검사S/W jobRef 전달 경로 ② 단일 NTP ③ 검사 S/W export 스키마(jobRef+UTC timestamp+도면좌표). ACS는 여전히 이미지·판정 미취급(ADR-004 경계 불변). inspection_result 기록 코드(writer)는 미구현 상태라 job_ref 채움은 후속 배선 대상
