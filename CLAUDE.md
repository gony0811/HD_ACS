...
- 2026-08-27: ACS↔AMR 통신 프로토콜 두절/재접속/재동기화 E2E 하네스 — 기존 SimTest(S1 앵커·S2 파라미터·S3 실패주입)가 통신 프로토콜 견고성은 검증하지 않던 공백을 메움. **시뮬레이터**(HD.Acs.Simulator/Program.cs)를 재접속 가능 구조로 리팩터(인라인 핸들러/접속을 `OnMessageAsync`·`ConnectAndAnnounceAsync`·`DropAndReconnectAsync` 로컬함수로 추출, `client` 가변화): ① 하네스 전용 제어 채널 `acs-sim/control/{mfr}/{serial}`(VDA 5050 외부) — `{"cmd":"drop","downMs":N}` 수신 시 `client.Dispose()`로 소켓 급단절→브로커가 retain된 Last Will(connection=CONNECTIONBROKEN) 발행→downMs 후 자동 재접속·재구독·ONLINE+state 재발행, ② `state`를 **retain 발행**으로 전환(마스터 재기동/재접속 시 진행 중 Order 즉시 회수 = ADR-002 재동기화 근거), ③ `PublishAsync`를 connected 게이트+try/catch로 가드(급단절 창에서 발행 스킵, 진행 중 Order 태스크는 메모리에서 계속 진행→재접속 후 이어서 완료 발행). **SimTest**(HD.Acs.SimTest/Program.cs)에 connection 토픽 구독+관측 순서 추적 추가하고 S4~S6 신설: S4 conn-lifecycle(구독 즉시 retain ONLINE 회수), S5 disconnect(제어 drop→CONNECTIONBROKEN 관측→자동 ONLINE 복귀), S6 reconnect-sync(4노드 Order 진행 중 두절 주입→재접속 후 전 액션 FINISHED 수렴+state.orderId=SIMTEST-S6 보존). run_simtest.sh 주석을 6개 시나리오로 갱신. Simulator/SimTest 빌드 0 error, **라이브 브로커(localhost:1883) E2E 6/6 PASS**(sim 로그로 mid-order 두절→Last Will→재접속→retain state 재동기화→Order 완료 육안 확인). VDA 5050 메시지 계약·마스터 클라이언트·상태머신·DB 무변경(테스트 계층만). 후속: 마스터측(Vda5050MasterClient) 자동 재접속 미구현(현재 기동 시 1회 접속)·스키마 위반 알람 TODO·factsheet는 별건
